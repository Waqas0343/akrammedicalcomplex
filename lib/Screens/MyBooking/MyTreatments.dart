...
class _MyTreatmentsState extends State<MyTreatments> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("My Home Services",),
      ),
      body: treatmentsModel.isNotEmpty
          ? treatmentsView()
          : Center(child: Text("No Services", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),),
    ));
  }
            ...
            itemBuilder: (BuildContext context, int index){
              ...
              return Card(
                ...
                child: Column(
                  children: [
                    ListTile(
                      title: AutoSizeText("Order Id # "+treatmentData.id, style: TextStyle(fontWeight: FontWeight.bold),maxLines: 1,),
                      subtitle: Text(treatmentData.status, style: TextStyle(color: treatmentData.getStatusColor(),),),
                      trailing: Icon(treatmentData.isExpanded ? Icons.keyboard_arrow_down :Icons.keyboard_arrow_up),
                      onTap: (){
                        setState(() {
                          treatments[index].isExpanded = !treatmentData.isExpanded;
                        });
                      },
                    ),
                    Visibility(
                        ...
                        child: ListView.builder(
                          ...
                          shrinkWrap: true,
                          itemBuilder: (BuildContext context, int index) {
                            ServiceModel model = treatmentData.details[index];
                            return ListTile(
                              dense: true,
                              title: Text(model.name),
                              subtitle: Text("Rs/- "+model.fee),
                            );
                          },
